...
def get_signal_info(ticker, qqq_mdd):
    if ticker == 'TQQQ':
        ...
        if -20 < qqq_mdd <= -15:
            ...
            badge = '<div class="status-badge">' + '☕ 현재 QQQ 타점 아님 (관망/홀딩)' + '</div>'
            msg = "☕ QQQ: 관망/홀딩"
            return (badge, msg, False)
...
if __name__ == '__main__':
    ...
    if not qqq_data:
        print("❌ QQQ 시세 수집 실패로 프로그램을 종료합니다.")
        exit()
    ...
    for t in TICKERS:
        ...
        if d:
            ...
            if is_active:
                ...
                telegram_lines.append(f"\n💡 <b>[{d['ticker']} 액션 시그널 발생!]</b>")
                telegram_lines.append(f"└ {tg_text}")
            
            card_item = (
                '<div class="ticker-card">'
                + f'<div class="ticker-title"><span>{d["ticker"]}</span> <small style="font-size: 13px; color: #6c757d;">52주 최고: ${d["ath"]}</small></div>'
                + f'<div><span class="price-current">${d["current"]}</span><span class="price-change {color_class}">{sign}{d["change"]} ({sign}{d["percent"]}%)</span></div>'
                + f'<ul class="stats-list"><li><span>전일 종가</span> <strong>${d["prev"]}</strong></li><li><span>52주 최고가 대비 (MDD)</span> <strong class="mdd-highlight">{d["mdd"]}%</strong></li></ul>'
                + html_badge
                + '</div>'
            )
            cards_html += card_item
        else:
            cards_html += '<div class="ticker-card"><div class="ticker-title"><span>' + t + '</span></div><p style="color:red;">데이터 실패</p></div>'
    ...
    # HTML 파일 생성 (다중 문자열 리스트로 줄바꿈 에러 완벽 차단)
    html_parts = [
        '<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>나스닥 마스터 분할 매매 시스템</title>',
        ...
        f'<div class="header"><h2 style="font-size: 18px; margin: 0;">🛰️ 실시간 매매 시그널 모니터링</h2><span style="font-size: 13px; color: #6c757d;">⏰ 시세 기준: {now_str}</span></div>',
